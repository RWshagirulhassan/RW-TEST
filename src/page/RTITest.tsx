...
import { RtiQuestionList } from "../context/Question";
...
export default function RTITest() {
  ...
  return (
    <div className="absolute min-h-screen h-full w-full gradient-mobile md:gradient-desktop">
      <div className="h-screen   flex flex-col  md:flex-row">
        <div className=" flex items-center justify-center h-[20%]  md:h-screen  w-[80%]   md:w-[35%] bg-[#26D1D412]  bg-opacity-[12%]  ">
          <img
            alt="carouselimg "
              ...
              <div className="flex flex-col px-[7%] h-full justify-between ">
                ...
                <ul className="w-full h-full flex flex-col justify-end text-[#ECFCFD]">
                  ...
                  {/* <button
                    onClick={nextIndex}
                    type="submit"
                    className="bg-[#42e6e9] text-[#1B1B1B]  w-full h-11 md:h-16 text-base font-semibold  md:text-2xl   flex items-center justify-center rounded-lg   py-4 disabled:bg-[#404040]"
                  >
                    NEXT
                  </button> */}
                  <Button
                    ...
                    onClick={() => {
                      if (!rtilist[index]?.selectedOption) return; // safety check
                      if (isLastQuestion) {
                        navigate("/RTI-REPORT"); // ✅ Replace with your final route
                      } else {
                        nextIndex(); // ✅ Go to the next question
                      }
                    }}
                    variant={
