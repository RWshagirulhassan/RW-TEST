...
export function calculateFutureValue(isSIP: boolean, payment: number, periods: number, ratePercent: number): number {
    ...
    switch (asset.title) {
      case 'Rental Income': {
        ...
        if (rupees > 0) {
          ...
          const fv = (rupees * ((1 + growthRate / 100) ** n - 1)) / (growthRate / 100)
          totalAssetValue += fv
        }
        break
      }
      ...
      case 'Portfolio': {
        ...
        if (debt > 0) {
          const fvDebt = (debt * ((1 + debtGrowth / 100) ** n - 1)) / (debtGrowth / 100)
          totalAssetValue += fvDebt
        }
        if (equity > 0) {
          const fvEquity = (equity * ((1 + equityGrowth / 100) ** n - 1)) / (equityGrowth / 100)
...
export function retirementPlan(
  ...
  download = true,
  annualInvestment = 0,
  allocationPercent: { Equity: number | null; Debt: number | null; Gold: number | null } = {
    Equity: null,
    Debt: null,
    Gold: null,
  },
): { data: (string | number)[][]; surplusOrShortfall: number; surplusOrShortfallByCPI: number } {
  ...
  assetCategories.forEach((asset) => {
    ...
    } else if (asset.title === 'Rental Income' || asset.title === 'Business Income' || asset.title === 'Others') {
  ...
  for (let i = 0; i <= totalYears; i += 1) {
    ...
    const passiveEquityGrowth = assetValues['Passive Equity'] * fixedGrowth['Passive Equity']
    ...
    const passiveDebtGrowth = assetValues['Passive Debt'] * fixedGrowth['Passive Debt']
    ...
    const committedEquityFlow = year < retirementAge ? newEq + eqGrowth : eqGrowth
    const committedDebtFlow = year < retirementAge ? newDebt + debtGrowth : debtGrowth
    const committedGoldFlow = year < retirementAge ? newGold + goldGrowth : goldGrowth
  ...
  const grossAssets = Object.keys(totals).reduce((acc, key) => {
    if (!['Year', 'Annual Expense', 'Expense CPI'].includes(key)) acc += totals[key]
    return acc
  }, 0)
  const difference = grossAssets - grossExpense
  const differenceOfCPI = grossAssets - grossExpenseByCPI

  data.push(['Gross Expense', formatForDisplay(grossExpense)])
  data.push(['Total Assets', formatForDisplay(grossAssets)])
  data.push(['Surplus / Shortfall', formatForDisplay(difference)])
